...
void AMenuSystemCharacter::CreateGameSession()
{
	...
	if (!OnlineSessionInterface.IsValid())
	{
	 ...
	 // Check if session exists and delete if it exists
	...
	//Add delegate to delegate list before creating session
	...
	SessionSettings->Set(FName("MATCHTYPE"), FString("FreeForAllNitin"), EOnlineDataAdvertisementType::ViaOnlineServiceAndPing);

	SessionSettings->bUseLobbiesIfAvailable = true;


	//Getting the first local player in the game for UNique Net id to be passed for session creation.
...
void AMenuSystemCharacter::OnCreateSessionComplete(FName SessionName, bool bWasSuccessful)
{
	if (GEngine && bWasSuccessful)
	{
		...
		GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Red, FString::Printf(TEXT("Created game sessioon %s"), *SessionName.ToString()));

	}
	else
	{
		GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Red, FString::Printf(TEXT(" NOT sessioon ")));
...
{					   
	...
	//Add delegate to delegate list before Finding session
...
void AMenuSystemCharacter::OnFindSessionComplete(bool bWasSuccessful)
{
	...
	for (auto Results : SessionSearch->SearchResults)
	{
		...
		FString name = Results.Session.OwningUserName;
		FString MatchType;
		Results.Session.SessionSettings.Get(FName("MATCHTYPE"), MatchType);
		if (GEngine && bWasSuccessful)
		{

			GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Cyan, FString::Printf(TEXT("join game sessioon %s and %s and %s"), *ID, *name, *MatchType));
		...
		if (MatchType == FString("FreeForAllNitin"))
		{
			if (GEngine)
			{

				GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Cyan, FString::Printf(TEXT("joining game sessioon %s"), *MatchType));
